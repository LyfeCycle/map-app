...
NSString * LyfeCycle_VersionTwo$ModuleRequireFormat = @"(function(exports){"
...
@implementation LyfeCycle_VersionTwoObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_lyfecycle_versiontwo);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create LyfeCycle_VersionTwo global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "LyfeCycle_VersionTwo" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_lyfecycle_versiontwo = [[LyfeCycle_VersionTwoObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_lyfecycle_versiontwo];
	
	NSString *_lyfecycle_versiontwoNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _lyfecycle_versiontwoNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_lyfecycle_versiontwo valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:LyfeCycle_VersionTwo$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.lyfecycle_versiontwo.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.lyfecycle_versiontwo.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
